import { Component, OnInit, OnDestroy } from '@angular/core';
import { Subscription } from 'rxjs';
import { delay } from 'rxjs/operators';
import Swal from 'sweetalert2';


import { Journalist } from 'src/app/models/journalist.model';
import { JournalistService } from 'src/app/services/journalist.service';

// import { BusquedasService } from '../../../services/busquedas.service';
// import { HospitalService } from '../../../services/hospital.service';
// import { ModalImagenService } from '../../../services/modal-imagen.service';

@Component({
  selector: 'app-journalists',
  templateUrl: './journalists.component.html',
  styles: [
  ]
})
export class JournalistsComponent implements OnInit, OnDestroy {

  public journalists: Journalist[] = [];
  public cargando: boolean = true;
  // private imgSubs: Subscription;

  constructor( private journalistService: JournalistService ) { }

  ngOnDestroy(): void {
    // this.imgSubs.unsubscribe();
  }

  ngOnInit(): void {
    this.cargarJournalists();

    // this.imgSubs = this.imgSubs = this.modalImagenService.nuevaImagen
    //   .pipe(delay(100))
    //   .subscribe( img => this.cargarHospitales() );
  }

  buscar( termino: string ) {

    if ( termino.length === 0 ) {
      return this.cargarJournalists();
    }

    // this.busquedasService.buscar( 'hospitales', termino )
    //     .subscribe( resp => {

    //       this.hospitales = resp;

    //     });
  }

  cargarJournalists() {

    this.cargando = true;
    this.journalistService.loadJournal()
        .subscribe( journalists => {
          this.cargando = false;

          this.journalists = journalists;
        })

  }

  guardarCambios( journalist: Journalist ) {

    this.journalistService.create( journalist.name, journalist.email || '' )
        .subscribe( resp => {
          Swal.fire( 'Actualizado', journalist.name, 'success' );
        });

  }

  // eliminarHospital( hospital: Hospital ) {

  //   // this.hospitalService.borrarHospital( hospital._id )
  //   //     .subscribe( resp => {
  //   //       this.cargarHospitales();
  //   //       Swal.fire( 'Borrado', hospital.nombre, 'success' );
  //   //     });

  // }

  async abrirSweetAlert() {
    const { value = '' } = await Swal.fire<string>({
      title: 'Crear hospital',
      text: 'Ingrese el nombre del nuevo hospital',
      input: 'text',
      inputPlaceholder: 'Nombre del hospital',
      showCancelButton: true,
    });

    // if( value.trim().length > 0 ) {
    //   this.hospitalService.crearHospital( value )
    //     .subscribe( (resp: any) => {
    //       this.hospitales.push( resp.hospital )
    //     })
    // }
  }

  // abrirModal(hospital: Hospital) {

  //   // this.modalImagenService.abrirModal( 'hospitales', hospital._id, hospital.img );

  // }

}
